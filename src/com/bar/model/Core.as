package com.bar.model
{
	...
	public class Core extends EventDispatcher
	{
		...
		/**
		 * Текущий бар, который показывается
		 */
		public var currentBar: BarPlace;
		/**
		 * Бар хозяина, того кто запустил приложение
		 */
		public var myBarPlace: BarPlace;
		/**
		 * Бар друга, пользователя, к которому зашли в гости
		 */
		public var userBarPlace: BarPlace;
		public var timer: Timer;
		public var maxProductionCount: int;
...
//		public var clientPeriodTimeStamp: Number;
		...
		public function Core(viewerId: String, srv: Server, maxProdCount: int)
		{
			maxProductionCount = maxProdCount;
			isLoaded = false;
			myBarPlace = new BarPlace(new User(viewerId));
			server = srv;
			if (server) {
				// если что, то сервер пришлет уведомление, что запуск первый
				firstLaunch = false;
				server.addEventListener(ServerEvent.EVENT_BAR_LOADED, onBarLoaded);
			}
			else {
		...
		public function setUserAttrs(fullName: String, photoPath: String): void {
			myBarPlace.user.fullName = fullName;
		...
		public function onTimer(event: TimerEvent): void {
			//если текущий бар - наш, то игровой процесс запущен, иначе только просмотр бара друга
			if (isLoaded && (currentBar == myBarPlace)) {
		...
		public function loadBar(id_user: String): void {
			//если грузим то, что уже сейчас открыто
			if (!currentBar || currentBar.user.id_user != id_user) {
				if (currentBar && (myBarPlace && myBarPlace.user.id_user == id_user)) {
					//проверить если свой бар уже загрузен - то просто показвать
					currentBar = myBarPlace;
					...
					if (server) {
						...
						barLoadEvent = new CoreEvent(CoreEvent.EVENT_BAR_LOADED);
		...
		public function getClientById(idClient: Number): Client {
			//используем currentBar - т.к. определение клиента нужно в любом виде бара
			for each(var c: Client in currentBar.clients) {
				if (c.id == idClient) {
					return c;
				}
			}
...
//			addedStartProduction = true;
			...
			if (server) {
				Bar.showDebug("userAttrsChanged");
		...
		public function getProductionById(idProduction: Number): Production {
			//Используем currentBar для получение продукции в любом баре
			for each(var p: Production in currentBar.production) {
				if (p.id == idProduction) {
					return p;
				}
			}
		...
		public function buyProduction(typeProduction: ProductionType): Boolean {
			if (currentBar == myBarPlace) {
				if (myBarPlace.production.length >= maxProductionCount) {
					return false;
				}
		...
		public function buyDecor(typeDecor: DecorType): Boolean {
			if (currentBar == myBarPlace) {
				...
				if (myBarPlace.user.level >= typeDecor.accessLevel &&
					...
					for each (decor in myBarPlace.decor) {
					...
					changeUserExp(typeDecor.expCount);
					if (server) {
		...
		/**
		 * Определяет, какой бар сейчас показан
		 */
		public function myBarShowed(): Boolean {
			return currentBar == myBarPlace;
		}
		
		//--------------------------------------------------------------------
		// ServerEvents
		//--------------------------------------------------------------------
		protected function onBarLoaded(event: ServerEvent): void {
			Bar.showDebug("Code.barloaded");
			var barLoadEvent: CoreEvent = new CoreEvent(CoreEvent.EVENT_BAR_LOADED);
			if (event.barPlace.user.id_user == myBarPlace.user.id_user) {
				if (event.firstLaunch) {
					Bar.showDebug("First.launch");
					firstLaunch = true;
					firstClientServed = false;
					initStartParams();
				}
				else {
					Bar.showDebug("Init myBarPlace");
					...
					myBarPlace.user.startMessages = event.barPlace.user.startMessages;
				}
				barLoadEvent.barPlace = myBarPlace;
				currentBar = myBarPlace;
				isLoaded = true;
				...
				Bar.sendGAStat("/userBarLoaded");
				barLoadEvent.barPlace = event.barPlace;
				userBarPlace = event.barPlace;
				currentBar = event.barPlace;
			}
			dispatchEvent(barLoadEvent);
		}
	}
