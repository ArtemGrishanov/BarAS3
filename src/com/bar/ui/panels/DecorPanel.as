package com.bar.ui.panels
{
	...
	public class DecorPanel extends GameLayer
	{
		...
		public function DecorPanel(value: GameScene, decorType: DecorType, menu: MainMenuPanel)
		{
			...
			bitmap = Images.getImage(Images.SHOP_PANEL);
			enabled = true;
			
			if (decorType.iconBitmap) {
	    		decorBitmap = decorType.iconBitmap;
			...
			nameTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 10, 0xffffff);
			...
			loveTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 10, 0xffffff);
			...
			levelTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 10, 0xffffff);
			...
			tipsTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 10, 0xffffff);
			...
			priceTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 10, 0xffffff);
			priceTf.autoSize = TextFieldAutoSize.LEFT;
			priceTf.x = PRICE_TF_X;
			addChild(priceTf);
			
			loveBitmap = Images.getImage(Images.SHOP_LOVE);
			loveBitmap.x = LOVE_ICON_CENTER_X - loveBitmap.width / 2;
			loveBitmap.y = LOVE_ICON_CENTER_Y - loveBitmap.height / 2;
			addChild(loveBitmap);
			
			levelBitmap = Images.getImage(Images.SHOP_LEV_PLUS);
			...
			tipsBitmap = Images.getImage(Images.SHOP_TIPS);
			...
			if (decorType.priceEuro > 0) {
				...
				moneyBitmap = Images.getImage(Images.SHOP_EURO);
			}
			else {
				priceTf.text = decorType.priceCent.toString();
				moneyBitmap = Images.getImage(Images.SHOP_CENTS);
			}
		...
		public function set enabledDecor(value: Boolean): void {
			...
			if (enabled) {
				...
				bitmap = Images.getImage(Images.SHOP_PANEL_GRAY);
				setSelect(false);
				setHover(false, false);
			}
		}
