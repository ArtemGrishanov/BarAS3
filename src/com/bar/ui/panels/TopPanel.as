package com.bar.ui.panels
{
	...
	/**
	 * Верхняя полоса с характеристиками пользователя (левел, ава, деньги и т.д.)
	 */
...
//		private var vk: VKontakte;
		...
		public function TopPanel(value:GameScene)
		{
			...
			bitmap = Images.getImage(Images.TOOLBAR_BACK);
			
			nameTf = new TextField();
			nameTf.autoSize = TextFieldAutoSize.LEFT;
            nameTf.selectable = false;
			nameTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 12, 0xffffff);
			nameTf.x = NAME_TF_X;
			nameTf.y = NAME_TF_Y;
			addChild(nameTf);
			...
			centsTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 12, 0xffffff);
			...
			euroTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 12, 0xffffff);
			euroTf.x = EURO_TF_X;
			euroTf.y = EURO_TF_Y;
			addChild(euroTf);
			...
			loveTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 12, 0xffffff);
			loveTf.autoSize = TextFieldAutoSize.LEFT;
			loveTf.x = LOVE_TF_X;
			loveTf.y = LOVE_TF_Y;
			addChild(loveTf);
			...
			levelTf.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 14, 0xffffff, true);
			levelTf.text = '1';
			...
			levelIcoGameObject.bitmap = Images.getImage(Images.TOOLBAR_LEV_ICO);
...
//			centsIcoGameObject.addEventListener(GameObjectEvent.TYPE_MOUSE_CLICK, licenseButtonClick);
			...
			centsIcoGameObject.bitmap = Images.getImage(Images.TOOLBAR_CENTS);
...
//			centsIcoGameObject.addEventListener(GameObjectEvent.TYPE_MOUSE_CLICK, licenseButtonClick);
			...
			euroIcoGameObject.bitmap = Images.getImage(Images.TOOLBAR_EURO);
...
//			centsIcoGameObject.addEventListener(GameObjectEvent.TYPE_MOUSE_CLICK, licenseButtonClick);
			...
			loveIcoGameObject.bitmap = Images.getImage(Images.TOOLBAR_HEART);
...
//			centsIcoGameObject.addEventListener(GameObjectEvent.TYPE_MOUSE_CLICK, licenseButtonClick);
			...
			//todo
			emptyExpBitmap = Images.getImage(Images.TOOLBAR_LEV_BACK);
			...
			blickBitmap.bitmap = Images.getImage(Images.TOOLBAR_LEV_BLICK);
			...
			t.selectable = false;
			t.defaultTextFormat = new TextFormat(Bar.DEFAULT_FONT, 11, 0xffffff, true);
...
//			vk.getProfiles([Bar.viewer_id]);
		...
		public function set photoUrl(url: String): void {
			_photoUrl = url;
			if (_photoUrl) {
				var b: Bitmap = multiLoader.get(_photoUrl);
				if (b) {
					avatar = b;
				}
				else {
					multiLoader.load(_photoUrl, _photoUrl, 'Bitmap');
		...
		public function set userName(value: String): void {
			if (value.length > MAX_NAME_LENGTH) {
				nameTf.text = value.substr(0, MAX_NAME_LENGTH) + '...';
			}
			else {
				nameTf.text = value;
			}
