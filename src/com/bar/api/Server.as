package com.bar.api
{
	...
	public class Server extends EventDispatcher
	{
		...
		public function connect(login: String, password: String): void {
			...
			Bar.showDebug("connect: " + _login + " " + _password);
		}
		...
		/**
		 * Говорит о том, что текущий пользователь вошел по приглашению пользователя id_friend
		 * Пользователю id_friend будет начислен бонус.
		 */
		public function enterByInvite(id_friend: String): void {
			var buf: ByteArray = new ByteArray();
			buf.writeInt(ServerProtocol.C_ENTER_BY_INVITE);
		...
		/**
		 * votes - голоса в сотых
		 */
		public function withdrawVotes(votes: int, item: String, authKey: String): void {
			Bar.showDebug("withdrawVotes: " + item + " " + votes);
			var buf: ByteArray = new ByteArray();
...
//		}
		...
		public function onLoginSuccess(event: SgsEvent): void {
			...
			var eventConnected: ServerEvent = new ServerEvent(ServerEvent.EVENT_LOGIN_SUCCESS);
	        dispatchEvent(eventConnected);
		}
		
		public function onLoginFailure(event: SgsEvent): void {
			_state = STATE_DISCONNECTED;
			debug("onLoginFailure " + event.failureMessage);
			_client.logout(true);
		}
	            	...
	            	case ServerProtocol.S_BAR_LOADED:
	            		...
	            		for (var i: int = 0; i < Balance.maxClientsCount; i++) {
	            			...
	            			if (cId >= 0) {
		            			...
		            			var clientType: ClientType = Balance.getClientTypeByName(cType);
	            				if (clientType) {
	            					var c: Client = new Client(clientType, cUserId, cName, false, new Date().getTime() / 1000, Balance.getGoodsTypeByName(cGoodsType), i);
	            					c.id = cId;
	            					clients.push(c);
//	            					debug('[S_BAR_LOADED]: Pos:' + i + ' Client:' + c.name + '(' + c.id + ') ' + c.id_vk + ' ' + c.typeClient.type + ' ' + c.orderGoodsType.type);
	            		...
	            		if (prodCount > 0) {
	            			for (i = 0; i < prodCount; i++) {
	            				...
	            				var productionType: ProductionType = Balance.getProductionTypeByName(prodType); 
	            				if (productionType) {
	            					var p: Production = new Production(productionType, partsCount);
		            				p.id = prodId;
...
//									debug('[S_BAR_LOADED]: Production:(' + p.id + ') ' + p.typeProduction.type + ' ' + p.partsCount + ' [' + p.cellIndex + ',' + p.rowIndex + ']');
	            		...
	            		if (decorCount > 0) {
	            			for (i = 0; i < decorCount; i++) {
	            				...
	            				var decorTypeName: String = buf.readUTF();
	            				var decorType: DecorType = Balance.getDecorTypeByName(decorTypeName); 
	            				if (decorType) {
	            					var d: Decor = new Decor(decorType);
	            					d.id = decorId;
		            				decor.push(d);
//									debug('[S_BAR_LOADED]: Decor:(' + d.id + ') ' + d.typeDecor.type);
						...
						//получеини start_messages
						var startMessagesCnt: int = buf.readInt();
						var startMessages: Array = new Array();
						if (startMessagesCnt > 0) {
							for (i = 0; i < startMessagesCnt; i++) {
								var msgId: int = buf.readInt();
								var caption: String = buf.readUTF();
								var message: String = buf.readUTF();
								var buttons: String = buf.readUTF();
								startMessages.push(new StartMessage(msgId, caption, message, buttons));
							}
						}
	            		...
	            		bp.decor = decor;
						bp.user.startMessages = startMessages;
...
//	            	break;
	            	...
	            	break;
	            	case ServerProtocol.S_WITHDRAW_VOTES_OK:
	            		var eventWithdrawVotesOk: ServerEvent = new ServerEvent(ServerEvent.EVENT_WITHDRAW_VOTES_OK);
	            		eventWithdrawVotesOk.votes = buf.readInt();
	            		eventWithdrawVotesOk.item = buf.readUTF();
	            		dispatchEvent(eventWithdrawVotesOk);
	            	break;
	            	case ServerProtocol.S_WITHDRAW_VOTES_NOT_ENOUGH:
						var eventWithdrawVotesNotEnough: ServerEvent = new ServerEvent(ServerEvent.EVENT_WITHDRAW_VOTES_NOT_ENOUGH);
						eventWithdrawVotesNotEnough.votes = buf.readInt();
						eventWithdrawVotesNotEnough.item = buf.readUTF();
						eventWithdrawVotesNotEnough.userBalance = buf.readInt();
	            		dispatchEvent(eventWithdrawVotesNotEnough);
	            	break;
	            	case ServerProtocol.S_WITHDRAW_VOTES_ERROR:
	            		var eventWithdrawVotesError: ServerEvent = new ServerEvent(ServerEvent.EVENT_WITHDRAW_VOTES_ERROR);
	            		dispatchEvent(eventWithdrawVotesError);
	            	break;
	            	case ServerProtocol.S_BONUS_FROM_INVITE:
	            		var eventBonusFromInvite: ServerEvent = new ServerEvent(ServerEvent.EVENT_BONUS_FROM_INVITE);
	            		var countInvitedUsers: int = buf.readInt();
	            		eventBonusFromInvite.invitedIds = new Array();
	            		eventBonusFromInvite.invitedNames = new Array();
	            		eventBonusFromInvite.invitedPhotoPathes = new Array();
	            		for (i = 0; i < countInvitedUsers; i++) {
	            			eventBonusFromInvite.invitedIds.push(buf.readInt());
	            			eventBonusFromInvite.invitedNames.push(buf.readUTF());
	            			eventBonusFromInvite.invitedPhotoPathes.push(buf.readUTF());
	            		}
	            		dispatchEvent(eventBonusFromInvite);
	            	break;
	            	default:
