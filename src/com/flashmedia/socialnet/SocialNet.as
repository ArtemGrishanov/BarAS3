package com.flashmedia.socialnet
{
	...
	public class SocialNet extends EventDispatcher
	{
		...
		public function SocialNet(viewerId: String, _net: String = '', _debug: Boolean = false, target: IEventDispatcher = null)
		{
			super(target);
			viewer_id = viewerId;
			debug = _debug;
			...
			switch (net) {
				...
				case NET_SANDBOX:
					...
					for (var i: uint = 0; i < uids.length; i++) {
						var u: SocialNetUser = new SocialNetUser(uids[i] as String, 'Nickname' + i, 'FirstName' + i, 'LastName' + i, SocialNetUser.SEX_MALE, '01.01.1990');
						users.push(u);
					}
...
//					var friendsIds: Array = [1234239, 4546611];
		...
		private function onVkontakteRequestError(event: VKontakteEvent):void {
			...
			var eventErr: SocialNetEvent = new SocialNetEvent(SocialNetEvent.ERROR);
			eventErr.errorCode = event.errorCode;
			eventErr.errorMessage = event.errorMessage;
			dispatchEvent(eventErr);
		}
				...
				switch (event.method) {
					case 'getProfiles':
						...
						for each (var o: Object in response) {
							var uid: String = (o.hasOwnProperty('uid') ? o.uid : null);
